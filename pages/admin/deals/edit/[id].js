...
export default function EditDeal() {
  ...
  useEffect(() => {
    if (id) {
      const foundDeal = vpsDealsData.find(d => d.id === id);
      if (foundDeal) {
        setDeal(foundDeal);
        // 填充表单字段
        setTitle(foundDeal.title);
        setDescription(foundDeal.description || '');
        setProviderId(foundDeal.providerId || '');
        setDiscountPercentage(foundDeal.discountPercentage?.toString() || '');
        setCouponCode(foundDeal.couponCode || '');
        setExpiryDate(foundDeal.expiryDate ? new Date(foundDeal.expiryDate).toISOString().split('T')[0] : '');
        setDealUrl(foundDeal.dealUrl || '');
        setFeatured(foundDeal.featured || false);
      } else {
        setMessage({ type: 'error', content: '未找到该优惠活动' });
      }
      setIsLoading(false);
    }
  }, [id]);
  ...
  const saveDeal = async (e) => {
    ...
    try {
      ...
      // 这里我们只是模拟保存成功
      setTimeout(() => {
        setMessage({ type: 'success', content: '优惠活动已成功更新' });
        setIsSaving(false);
      }, 1000);
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 flex items-center justify-center">
        <div className="text-white text-center">
          ...
          <h1 className="text-2xl font-bold mb-4">未找到优惠活动</h1>
          <p className="mb-6">您请求的优惠活动不存在或已被删除</p>
          <Link href="/admin/dashboard">
            <span className="px-4 py-2 bg-cyan-600 hover:bg-cyan-700 text-white font-medium rounded transition-colors">
              返回仪表盘
            </span>
          </Link>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800">
      <Head>
        <title>编辑优惠活动 - {title} | VPS推荐网管理后台</title>
        <meta name="description" content="编辑VPS优惠活动" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-orbitron font-bold text-cyan-400">编辑优惠活动: {title}</h1>
          <Link href="/admin/dashboard">
